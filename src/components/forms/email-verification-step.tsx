...
export default function EmailVerificationStep({ user, onComplete, isLoading, setIsLoading }: Props) {
  ...
  const sendOTP = async (data: EmailStepData) => {
    ...
    try {
      ...
      alert('Failed to send verification code: ' + (err instanceof Error ? err.message : 'Unknown error'))
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const verifyOTP = async (otpCode: string) => {
    ...
    try {
      ...
      alert('Invalid verification code. Please try again: ' + (err instanceof Error ? err.message : 'Unknown error'))
      setOtp(['', '', '', '', '', '']) // Clear OTP
    } finally {
      setIsLoading(false)
    }
  ...
  const resendOTP = async () => {
    ...
    try {
      ...
      alert('Failed to resend code: ' + (err instanceof Error ? err.message : 'Unknown error'))
    } finally {
      setIsLoading(false)
    }
  }
