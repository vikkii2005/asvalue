...
export default function SignInRedirect({
    ...
    async function initiateOAuth() {
      ...
      try {
        ...
        if (enableAnalytics) {
          ...
          console.log(
            `⚡ OAuth preparation completed in ${processingTime.toFixed(2)}ms`
          )

          // Update analytics session
          const authSession = JSON.parse(
            localStorage.getItem('asvalue_auth_session') || '{}'
          )
  ...
  if (error) {
    return (
      <div
        className={`auth-error-container ${className}`}
        style={{
          display: 'flex',
  ...
  return (
    <div
      className={`signin-redirect-container ${className}`}
      style={{
        display: 'flex',
        flexDirection: 'column',
        alignItems: 'center',
