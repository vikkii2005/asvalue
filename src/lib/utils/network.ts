...
// Network status detection (COMPLETELY FIXED VERSION)
...
export class NetworkMonitor {
  ...
  public static getNetworkInfo(): NetworkInfo {
    ...
    if (connection?.effectiveType)
      info.effectiveType =
        connection.effectiveType as NetworkInfo['effectiveType']
  ...
  public static async testConnectionSpeed(): Promise<{
    ...
    try {
      ...
      // ✅ FIXED: Prefixed with _ to suppress warning
      console.error('Connection speed test failed:', _error)
  ...
  public static isSuitableForOperation(
    operation: 'auth' | 'upload' | 'stream' | 'basic'
  ): boolean {
  ...
  private static setupEventListeners(): void {
    if (typeof window !== 'undefined') {
      ...
      const connection: NavConn | undefined =
        (navigator as unknown as { connection?: NavConn }).connection ||
        (navigator as unknown as { mozConnection?: NavConn }).mozConnection ||
        (navigator as unknown as { webkitConnection?: NavConn })
          .webkitConnection
  ...
  private static notifyListeners(): void {
    ...
    this.listeners.forEach(callback => {
      try {
        ...
        console.error('Network listener error:', _listenerError) // ✅ FIXED: Prefixed with _
      }
    })
  }
...
export const waitForOnline = (timeout?: number) =>
  NetworkMonitor.waitForOnline(timeout)
export const isSuitableForAuth = () =>
  NetworkMonitor.isSuitableForOperation('auth')
export const addNetworkListener = (cb: (networkInfo: NetworkInfo) => void) =>
  NetworkMonitor.addListener(cb)
