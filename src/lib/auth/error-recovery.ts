...
export class AuthErrorRecovery {
  ...
  public classifyError(
    ...
    if (this.isSecurityError(error)) {
      return {
        type: 'security',
        message:
          'Security validation failed. Please start a new authentication attempt.',
        retryable: false,
        severity: 'high',
        timestamp,
        context,
    ...
    return {
      type: 'unknown',
      message:
        this.extractErrorMessage(error) || 'An unexpected error occurred.', // ✅ FIXED: Renamed to avoid duplicate
      retryable: true,
      severity: 'medium',
      timestamp,
      context,
  ...
  public calculateRetryDelay(error: AuthError, attemptKey: string): number {
    ...
    let delay =
      strategy.initialDelay * Math.pow(strategy.backoffMultiplier, attempts)
  ...
  public async executeWithRetry<T>(
    ...
    for (
      let attempt = 0;
      attempt < this.defaultRetryStrategy.maxAttempts;
      attempt++
    ) {
      try {
  ...
  public getUserFriendlyMessage(authError: AuthError): string {
    const messages: Record<AuthError['type'], string> = {
      ...
      timeout:
        'The request took too long. Please try again with a better connection.',
  ...
  public getRetryStats(attemptKey: string): {
    attempts: number
    lastError?: AuthError
  } {
    return {
