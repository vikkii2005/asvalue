...
export default function ProfileSetup({ onComplete }: ProfileSetupProps) {
  ...
  // FIXED: Enhanced form submission with bulletproof error handling
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // FIXED: Enhanced user verification with better error handling
      const { data: existingUser, error: userError } = await supabase
        .from('sellers')
        .select('id, email')
        .eq('email', session.user.email)
        .maybeSingle() // ✅ FIXED: Won't error if 0 rows found

      setDebugInfo(`🔍 User lookup result: ${JSON.stringify({ existingUser, userError })}`)

      if (userError) {
        throw new Error(`Database error during user verification: ${userError.message}`)
      }

      if (!existingUser) {
        // FIXED: User doesn't exist - create them first
        setDebugInfo('⚠️ No user found, creating seller record...')
        ...
        setDebugInfo(`✅ New user created: ${newUser.id}`)
        
        // Now update the newly created user with profile data
        ...
        setDebugInfo(`✅ Profile created successfully: ${JSON.stringify(updatedData)}`)
      } else {
        ...
        const { data: updatedData, error: updateError } = await supabase
          ...
          .select('business_name, category, phone')
          .maybeSingle() // ✅ FIXED: Use maybeSingle here too
      ...
      const error = err as Error
      setDebugInfo(`💥 Profile save failed: ${error.message}`)
      
      // FIXED: More specific error messages for users
      let userMessage = 'Failed to save profile. Please try again.'
      
      if (error.message.includes('Cannot coerce')) {
        userMessage = 'Database connection error. Please refresh and try again.'
