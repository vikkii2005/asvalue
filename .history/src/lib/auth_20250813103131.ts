  ...
  callbacks: {
        ...
        if (account?.provider === 'google' && user.email) {
          ...
          if (!existingUser) {
            ...
            const { error } = await supabase
              ...
              .insert([
                {
                  // Let Supabase generate the UUID automatically (no user.id here)
                  email: user.email,
                  name: user.name || '',
    ...
    async jwt({ token, user, account }) {
      ...
      if (account && user?.email) {
        try {
          // Get the database user record using email (NOT user.id)
          ...
          if (dbUser) {
            // Store database UUID in token (NOT user.id)
            token.id = dbUser.id
