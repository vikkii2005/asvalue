...
export class OAuthConfigManager {
  ...
  private validateConfig(): void {
    ...
    if (!google.clientId) {
      throw new Error(
        'Google Client ID is required. Set NEXT_PUBLIC_GOOGLE_CLIENT_ID environment variable.'
      )
    }

    if (this.environment === 'production' && !google.clientSecret) {
      console.warn(
        'Google Client Secret is recommended for production environments.'
      )
    }

    if (
      !google.redirectUri.startsWith('https://') &&
      this.environment === 'production'
    ) {
      throw new Error(
        'HTTPS is required for OAuth redirect URIs in production.'
      )
    }

    console.log(
      `✅ OAuth configuration validated for ${this.environment} environment`
    )
  }
  ...
  public getUserInfoEndpoint(provider: keyof ProviderConfig): string {
    ...
    if (!endpoint) {
      throw new Error(
        `User info endpoint not configured for provider: ${provider}`
      )
    }
...
export const getGoogleTokenEndpoint = () =>
  oauthConfig.getTokenEndpoint('google')
export const getGoogleUserInfoEndpoint = () =>
  oauthConfig.getUserInfoEndpoint('google')
