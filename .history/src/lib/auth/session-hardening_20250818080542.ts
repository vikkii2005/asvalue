...
export class SessionHardening {
  ...
  public async rotateSessionTokens(sessionId: string): Promise<{ accessToken: string; refreshToken: string } | null> {
    try {
      ...
      // Generate new tokens (this would typically call your auth provider)
  ...
  private async calculateRiskScore(fingerprint: SessionFingerprint): Promise<number> {
    ...
    // e.g., checking against known malicious user agents, etc.

    return Math.min(score, this.maxRiskScore);
  }

  private detectDeviceType(userAgent: string): SessionSecurity['deviceType'] {
  ...
  private validateFingerprint(stored: SessionFingerprint, current: SessionFingerprint): boolean {
    ...
    // All critical factors must match, and at least 2 non-critical factors
  ...
  private async generateNewTokens(userId: string): Promise<{ accessToken: string; refreshToken: string }> {
    // This would typically refresh tokens through your auth provider
  ...
  private mapDbSessionToSessionSecurity(data: any): SessionSecurity {
    return {
      id: data.id,
      userId: data.user_id,
