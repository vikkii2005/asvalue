...
export class SecureStorage {
  ...
  public static setItem<T>(key: string, value: T, options: StorageOptions = {}): boolean {
    try {
      const {
        encrypt = false,
        expiration,
  ...
  public static getItem<T>(key: string, options: StorageOptions = {}): T | null {
    try {
...
export class SecureSessionStorage {
  ...
  public static setItem<T>(key: string, value: T, options: Omit<StorageOptions, 'expiration'> = {}): boolean {
    try {
      const { prefix = this.defaultPrefix } = options;
      const fullKey = `${prefix}${key}`;
  ...
  public static getItem<T>(key: string, options: Omit<StorageOptions, 'expiration'> = {}): T | null {
    try {
      const { prefix = this.defaultPrefix } = options;
      const fullKey = `${prefix}${key}`;
  ...
  public static removeItem(key: string, options: Omit<StorageOptions, 'expiration'> = {}): boolean {
    try {
      const { prefix = this.defaultPrefix } = options;
      const fullKey = `${prefix}${key}`;
...
// Convenience exports
export const setSecureItem = <T>(key: string, value: T, options?: StorageOptions) => 
