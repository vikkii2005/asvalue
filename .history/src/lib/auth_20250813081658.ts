...
export const authOptions: NextAuthOptions = {
  ...
  callbacks: {
        ...
        if (account?.provider === 'google' && user.email) {
          ...
          if (!existingUser) {
            ...
            const newSeller = await db.sellers.create({
              ...
              avatar_url: user.image || (profile as { picture?: string })?.picture || null,
              is_active: true,
              phone: null,
              category: null,
              business_name: null,
            ...
            const updatedSeller = await db.sellers.update(existingUser.id, {
              ...
              avatar_url: user.image || (profile as { picture?: string })?.picture || existingUser.avatar_url,
            })
    ...
    async jwt({ token, user, account }) {
      ...
      if (token.email) {
        try {
          const supabaseUser = await db.sellers.getByEmail(token.email as string)
          if (supabaseUser) {
...
export const getServerSession = async () => {
  const { getServerSession: getNextServerSession } = await import('next-auth/next')
