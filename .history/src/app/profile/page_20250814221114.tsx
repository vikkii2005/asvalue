...
export default function ProfilePage() {
  ...
  // ✅ ENHANCED SUBMIT HANDLER WITH FORCE REFRESH
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!profileData.business_name) {  // Fixed: no backslashes
      ...
      // ✅ FORCE REFRESH AUTH CONTEXT - This is the key fix!
      console.log('🔄 Refreshing auth context...');
      
      // Option 1: Try to refresh auth context first
      if (refreshProfile) {
        try {
          ...
          console.log('❌ Auth refresh failed, using force reload as fallback');
          // Fallback to force reload if refresh fails
          setTimeout(() => {
            window.location.href = '/dashboard';
          }, 1000);
        }
      } else {
        // Option 2: Force page reload (guaranteed to work)
        console.log('🔄 Force reloading to refresh auth context');
        setTimeout(() => {
          window.location.href = '/dashboard';
        }, 1000);
      }
